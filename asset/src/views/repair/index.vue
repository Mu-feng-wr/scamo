        ...
        <SearchArea :show-all-search.sync="showAllSearch" class="p-16">
          <div class="flex">
            <div class="searchLeft">
              <el-row :gutter="14">
                <el-col :span="4">
                  <el-input v-model="queryParams.assetRepairCode" size="mini" placeholder="维修单号" clearable @keyup.enter.native="load" />
                </el-col>
                <el-col :span="4">
                  <el-input v-model="queryParams.maintainerName" size="mini" placeholder="维修人" clearable @keyup.enter.native="load" />
                </el-col>
                <el-col :span="4">
                  <el-input v-model="queryParams.maintainerOrgName" size="mini" placeholder="维修部门" clearable @keyup.enter.native="load" />
                </el-col>
                <el-col :span="4">
                ...
                <el-col :span="4">
                  <el-input v-model="queryParams.repairReason" size="mini" placeholder="维修原因" clearable @keyup.enter.native="load" />
                </el-col>
              </el-row>
              <el-row v-show="showAllSearch" :gutter="14" class="mt-10">
                ...
                <el-col :span="4">
                  <base-input size="mini" base-code="AlmAssetRepair-status" :options-list="dictDataList" :value.sync="queryParams.status" placeholder="状态" clearable @change="load" />
                </el-col>
                <el-col :span="8">
                  <input-range
                    type="daterange"
                    ...
                    start-placeholder="维修时间开始"
                    end-placeholder="维修时间结束"
      ...
      <el-main style="padding:0;">
        <el-container>
          <el-header>
            <el-row class="mb-15">
              <el-col :span="12">
                <el-button v-hasPermi="['asset:repair:add']" type="primary" plain icon="el-icon-plus" size="mini" @click="addOrUpdateHandle()">新增维修单</el-button>
                <el-button v-hasPermi="['asset:repair:export']" plain icon="el-icon-upload2" size="mini" @click="handleExport">导出</el-button>
              </el-col>
          ...
          <el-main>
            ...
            >
              ...
              <template #assetRepairCode="{row}">
                <el-link type="primary" :underline="false" @click="detailHandle(row.assetRepairId)">{{ row.assetRepairCode }}</el-link>
              </template>
              ...
              <template #status="{row}">
                <dictDateView :value="row.status" :dict-data-list="dictDataList" dict-code="AlmAssetRepair-status" />
              </template>
              <template v-slot:todo="{ row }">
                <div class="todo">
                  <el-button size="mini" type="text" @click="detailHandle(row.assetRepairId)">查看</el-button>
                  <el-button v-if="row.status == 0 || row.status == 3" v-hasPermi="['asset:repair:edit']" size="mini" type="text" @click="addOrUpdateHandle(row.assetRepairId)">修改</el-button>
                  <el-button
                    v-if="row.status == 2 && (row.assetReviewAuditVO&&row.assetReviewAuditVO.processId=='DIRECT_SUPERIOR_APPROVAL')"
                    v-hasPermi="['asset:repair:audit']"
                    size="mini"
                    type="text"
                  ...
                  <el-button
                    ...
                    v-hasPermi="['asset:repair:recall']"
                    size="mini"
                    type="text"
                  ...
                  <el-button
                    ...
                    v-hasPermi="['asset:repair:register']"
                    size="mini"
                    type="text"
                  ...
                  <el-button
                    v-if="row.status == 2 && (row.assetReviewAuditVO&&row.assetReviewAuditVO.processId=='DIRECT_SUPERIOR_APPROVAL')"
                    v-hasPermi="['asset:repair:invalid']"
                    size="mini"
                    type="text"
                    @click="audit(row,'invalid_add')"
                  >作废</el-button>
                  <el-button v-if="row.status == 0 || row.status == 3 || row.status == 4" v-hasPermi="['asset:repair:remove']" size="mini" type="text" @click="handleDelete(row)">删除</el-button>
                </div>
              </template>
              <template #pager>
                <el-pagination
...
import { listRepair, delRepair } from '@/api/repair.js'
...
export default {
  ...
  data() {
    return {
      ...
      tableColumn: [
        ...
        { visible: true, field: 'assetRepairCode', title: '维修单号', width: 150, fixed: 'left', visibleDisabled: true, slots: { default: 'assetRepairCode' } },
        { visible: true, field: 'repairDate', title: '维修日期', width: 130, fixed: 'left', visibleDisabled: true },
        ...
        { visible: true, field: 'maintenanceTotalCost', title: '资产含税金额（元）', width: 160, headerAlign: 'center', align: 'right', formatter: 'formatMoney' },
        { visible: true, field: 'maintainerName', title: '维修人', width: 130 },
        { visible: true, field: 'maintainerOrgName', title: '维修部门', width: 130 },
        { visible: true, field: 'repairReason', title: '维修原因', width: 240, headerAlign: 'center', align: 'left' },
        { visible: true, field: 'assetAmount', title: '维修总金额（元）', width: 160, headerAlign: 'center', align: 'right', formatter: 'formatMoney' },
        ...
        { visible: true, field: 'maintainerCompanyName', title: '维修供应商', width: 200, headerAlign: 'center', align: 'left' },
  ...
  methods: {
    ...
    getFooterData({ columns, data }) {
      return this.footerMethod(columns, data, '', ['assetQuantity', 'maintenanceTotalCost', 'assetAmount'])
    },
    // 获取字典数据
    getDictData() {
      let dictCodes = 'System-sourceTerminal' // 来源终端
      dictCodes += ',AlmAssetRepair-status' // 维修状态
