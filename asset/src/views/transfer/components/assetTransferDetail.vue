<template>
  <div>
    <div v-if="showBtn" class="pt-10 mb-10">
      ...
      <setUser v-if="showBtn" :disabled="btnDisabled" size="mini" class="pr-10" @confirm="setUserHandler" />
      <selectShelf v-if="showBtn" :disabled="btnDisabled" size="mini" class="pr-10" @confirm="selectShelfHandler" />
    ...
    >
      ...
      <template #remarks="{row}">
        <el-input v-model="row.remarks" placeholder="备注" show-word-limit :maxlength="300" />
      </template>
...
export default {
  ...
  computed: {
    tableColumn() {
      return [
        ...
        { field: 'assetCode', fixed: 'left', title: '资产编号', width: 130 },
        ...
        { field: 'newOrgName', title: '新使用部门', width: 160 },
        ...
        { field: 'transferQuantity', title: '调拨数量', width: 130, headerAlign: 'center', align: 'right', formatter: 'formatMoney' },
        ...
        { field: 'taxRate', title: '税率（%）', width: 100, headerAlign: 'center', align: 'right', formatter: 'formatMoney' },
  ...
  methods: {
    ...
    selectStandbookHandler(list) {
      this.tableData = list.map((item) => {
        ...
        return {
          ...item,
          ...{
            transferQuantity: item.inventoryQuantity,
    ...
    async saveRow(row) {
      ...
      if (!errMap) {
        this.$set(row, 'subtotal', this.$vxe.multiply(row.transferQuantity, row.priceIncludTax))
    ...
    getFooterData({ columns, data }) {
      return this.footerMethod(columns, data, '', ['inventoryQuantity', 'transferQuantity', 'priceExcludTax', 'taxAmount', 'priceIncludTax', 'subtotal', 'discountAmount'], 1)
    }
  }
}
</script>
