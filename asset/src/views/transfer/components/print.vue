    ...
    <el-form ref="form" :model="form" label-width="95px" label-suffix="：" class="bg-white">
      <div id="printWrap" ref="demo" class="print_obj bgClass">
        ...
        <el-card class="box-card" style="page-break-after: always">
          <div slot="header" class="clearfix text-tip">{{ $store.getters.userInfo.transferDate }}</div>
          <div slot="header" class="clearfix text-tip">{{ $vxe.toDateString(printData.collectDate,'yyyy')+'年'+$vxe.toDateString(printData.collectDate,'MM')+'月' }}资产领用出库签收单</div>

          <el-row>
            <el-col :span="8">
              <el-form-item label="调拨单号">{{ printData.assetTransferCode }}</el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item label="调拨日期">{{ printData.transferDate }}</el-form-item>
            </el-col>
            <el-col :span="8">
          ...
          <el-row>
            <el-col :span="8">
              <el-form-item label="接收人">{{ printData.receiverName }}</el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item label="接收部门">{{ printData.receiverOrgName }}</el-form-item>
            </el-col>
            <el-col :span="8">
          ...
          <table class="el-table">
            ...
            <tbody>
              <tr v-for="(item, index) in printData.assetTransferDetail" :key="index">
                ...
                <td>
                  {{ item.newWarehouseName }}
                  {{ item.remarks?'-'+item.remarks:'' }}
                </td>
              </tr>
              <template v-if="printData.assetTransferDetail &&printData.assetTransferDetail.length < 3">
                <tr>
              ...
              <tr>
                <td>情况说明</td>
                <td colspan="5">
                  <div class="u-line-1">{{ printData.transferReason }}</div>
