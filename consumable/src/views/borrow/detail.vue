  ...
  <PageCard v-loading="submitLoading" :return-url="returnUrl">
    <el-form ref="form" label-width="170px" class="form-table" style="margin-bottom: 8px">
      <SectionCard title="基本信息">
        <el-row>
          <el-col :span="8">
            <el-form-item label="借用单号">{{ formData.consumableBorrowCode }}</el-form-item>
          </el-col>
          <el-col :span="8">
      ...
      <SectionCard title="申请信息" class="mt-8">
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="资产数量">{{ $vxe.commafy(formData.consumableQuantity,{ digits: 3 }) }}</el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="资产含税金额（元）">{{ $vxe.commafy(formData.consumableAmount,{ digits: 3 }) }}</el-form-item>
          </el-col>
          <el-col :span="8">
    ...
    <SectionCard title="资产明细">
      <consumableBorrowDetail ref="consumableBorrowDetail" :show-btn="false" :form-data="formData" />
    </SectionCard>
...
export default {
  ...
  methods: {
    init() {
      ...
      getBorrow(this.editId).then((res) => {
        this.formData = {
          ...res.data,
          ...{
    ...
    // 获取字典数据
    getDictData() {
      var dictCodes = 'AlmAssetBorrow-ynExpectReturn' // 借用是否需要归还
