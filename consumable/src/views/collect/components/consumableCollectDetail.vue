<template>
  <div>
    <div class="pt-1 mb-8">
      ...
      <el-button v-if="showBtn" size="mini" icon="el-icon-upload2" @click="exportAssetDeatil">导出</el-button>
      <el-button v-if="showBtn" icon="el-icon-delete" type="danger" size="mini" plain @click="handleDeleteBatch">删除</el-button>
    </div>
    <vxe-grid
      ref="xTable"
      v-loading="loading"
    ...
    >
      ...
      <template #firstShelfName="{row}">
        <span>{{ row.firstShelfName }}/{{ row.secondShelfName }}</span>
      </template>
      ...
      <template #collectQuantity="{ row }">
        <span v-if="!$refs.xTable.isActiveByRow(row)">{{ $vxe.commafy(row.collectQuantity,{digits:3}) }}</span>
        <el-input-number
          v-else
          v-model="row.collectQuantity"
          size="mini"
...
export default {
  ...
  computed: {
    tableColumn() {
      return [
        ...
        { field: 'collectQuantity', title: '领用数量', width: 120, slots: { default: 'collectQuantity', edit: 'collectQuantity' }, editRender: {} },
    ...
    standBookQuery() {
      return [{ labelName: 'centralizedBusinessName', valueName: 'centralizedBusinessId', label: this.formData.centralizedBusinessName, value: this.formData.centralizedBusinessId, name: '业务类别', require: true }]
    }
  },
  watch: {
    'formData.clmConsumableCollectDetailList': {
  ...
  methods: {
    ...
    getDictData() {
      ...
      dictCodes += ',AlmAssetCollect-usePurpose' // 资产物态
    ...
    selectStandbookHandler(list) {
      this.tableData = list.map((item) => {
        ...
        this.$set(item, 'schemeName', this.formData.schemeName)
        this.$set(item, 'collectQuantity', null)
        if (!item.subscriptionId) {
          item.discountAmount = undefined
        } else {
          this.$set(item, 'discountAmount', item.discountAmount)
        }
        return item
      })
      this.$emit('calculate', this.tableData)
    ...
    // 导出耗材资产明细
    exportAssetDeatil() {
      this.download('/consume/collectDetail/export', { consumableCollectId: this.formData.consumableCollectId }, `耗材领用明细_${new Date().getTime()}.xlsx`)
    }
  }
}
</script>
