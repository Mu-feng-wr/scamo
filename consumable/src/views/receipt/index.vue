        ...
        <SearchArea :show-all-search.sync="showAllSearch" class="p-16">
          <div class="flex">
            <div class="searchLeft">
              <el-row :gutter="14">
                <el-col :span="4">
                  <el-input v-model="queryParams.consumableReceiptCode" size="mini" placeholder="入库单号" clearable @keyup.enter.native="load" />
                </el-col>
                <el-col :span="4">
      ...
      <el-main style="padding:0;">
        <el-container>
          <el-header>
            <el-row class="mb-15">
              <el-col :span="12">
                <el-button v-hasPermi="['consume:receipt:add']" type="primary" plain icon="el-icon-plus" size="mini" @click="addOrUpdateHandle()">新增入库单</el-button>
                <el-button v-hasPermi="['consume:receipt:export']" plain icon="el-icon-upload2" size="mini" @click="handleExport">导出</el-button>
              </el-col>
          ...
          <el-main>
            ...
            >
              ...
              <template v-slot:consumableReceiptCode="{row}">
                <el-link type="primary" :underline="false" @click="detailHandle(row.consumableReceiptId)">{{ row.consumableReceiptCode }}</el-link>
              </template>
              ...
              <template v-slot:todo="{ row }">
                <div class="todo">
                  <el-button size="mini" type="text" @click="detailHandle(row.consumableReceiptId)">查看</el-button>
                  <el-button v-if="row.status == 0|| row.status == 3" v-hasPermi="['consume:receipt:edit']" size="mini" type="text" @click="addOrUpdateHandle(row.consumableReceiptId)">修改</el-button>
                  <el-button
                    v-if="row.status == 2 && (row.assetReviewAuditVO&&row.assetReviewAuditVO.processId=='DIRECT_SUPERIOR_APPROVAL')"
                    v-hasPermi="['consume:receipt:audit']"
                    size="mini"
                    type="text"
                  ...
                  <el-button
                    ...
                    v-hasPermi="['consume:receipt:recall']"
                    size="mini"
                    type="text"
                  ...
                  <el-button
                    ...
                    v-hasPermi="['consume:receipt:register']"
                    size="mini"
                    type="text"
                  ...
                  <el-button
                    v-if="row.status == 2 && (row.assetReviewAuditVO&&row.assetReviewAuditVO.processId=='DIRECT_SUPERIOR_APPROVAL')"
                    v-hasPermi="['consume:receipt:invalid']"
                    size="mini"
                    type="text"
                    @click="audit(row,'invalid_add')"
                  >作废</el-button>
                  <el-button v-if="row.status == 0 || row.status == 3 || row.status == 4" v-hasPermi="['consume:receipt:remove']" size="mini" type="text" @click="handleDelete(row)">删除</el-button>
                </div>
              </template>
              <template #pager>
                <el-pagination
  ...
  data() {
    return {
      ...
      tableColumn: [
        ...
        {
          visible: true,
          field: 'consumableReceiptCode',
          title: '入库单号',
          width: 150,
          fixed: 'left',
          visibleDisabled: true,
          slots: { default: 'consumableReceiptCode' }
        },
        ...
        { visible: true, field: 'assetCompanyName', title: '耗材所属公司', width: 200, headerAlign: 'center', align: 'left' },
