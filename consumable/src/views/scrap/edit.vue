<template>
  <PageCard v-loading="submitLoading" :return-url="returnUrl">
    <el-form ref="form" :model="formData" :rules="rules" label-width="180px" class="form-table form-table-edit" style="margin-bottom: 8px">
      ...
      <SectionCard title="申请信息" class="mt-8">
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="耗材数量" prop="assetQuantity">
          ...
          <el-col :span="8">
            <el-form-item label="终结方式">
              <el-input suffix-icon="el-icon-arrow-down" placeholder="请选择终结方式" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
    ...
    <SectionCard class="mt-8" title="耗材明细">
      <consumableScrapDetail ref="consumableScrapDetail" :form-data="formData" @calculate="calculate" />
    </SectionCard>
...
export default {
  ...
  methods: {
    ...
    submitForm(status, curProcessStepName) {
      this.$refs['form'].validate(async (valid) => {
        if (valid) {
          ...
          if (await this.$refs.consumableScrapDetail.validTable()) {
            return
          }
          this.formData.almAssetScrapDetailList = this.$refs.consumableScrapDetail.tableData.map((item, i) => {
            item.sort = i + 1
            return item
          })
          this.formData.status = status
          if (this.editId) {
            updateScrap(this.formData).then((response) => {
            ...
            addScrap(this.formData).then((res) => {
    ...
    approve(status, curProcessResult) {
      ...
      this.$refs['form'].validate(async (valid) => {
        if (valid) {
          if (this.$refs.consumableScrapDetail.tableData.length == 0) {
            this.$message({
              type: 'warning',
              message: '请添加明细表数据'
            })
          ...
          var submitData = {
            ...this.formData,
            ...{
              almAssetScrapDetailList: this.$refs.consumableScrapDetail.tableData.map((item, i) => {
                item.sort = i + 1
                return item
              }),
              status: status,
