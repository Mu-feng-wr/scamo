<template>
  <div>
    <div class="pt-10 mb-10">
      ...
      <uploadImg v-if="showBtn" size="mini" :disabled="btnDisabled" class="pl-10 pr-10" @success="uploadImgSuccess" />
      ...
      <selectShelf v-if="showBtn" size="mini" :disabled="btnDisabled" :query="{parentId:formData.warehouseId}" class="pr-10" @confirm="selectShelfHandler" />
...
<script>
import { listSchemeQuery, listDictItems, listAddressQuery, queryAddressTreeSelect, queryUnShippedByPurchaseApplicationId } from '@/api/base.js'
export default {
  props: {
    formData: {
      type: Object,
  ...
  computed: {
    tableColumn() {
      return [
        ...
        { field: 'materialCode', title: '耗材编码', width: 180, fixed: 'left' },
        { field: 'materialName', title: '耗材名称', width: 200, fixed: 'left' },
        { field: 'categoryName', title: '耗材分类', width: 150, fixed: 'left' },
        { field: 'pictureRui', title: '耗材图片', width: 250, showOverflow: true, slots: { default: 'pictureRui', edit: 'pictureRui' }, editRender: {} },
        ...
        { field: 'alterUnitNum', title: '库存数量', width: 120, visible: this.formData.accepterMethod == 1 },
        { field: 'buyQuantity', title: '调拨数量', width: 140, headerAlign: 'center', align: 'right', slots: { default: 'buyQuantity', edit: 'buyQuantity' }, editRender: {} },
        ...
        { field: 'subtotal', title: '金额小计', width: 160, formatter: 'formatMoney', headerAlign: 'center', align: 'right' },
        ...
        { field: 'assetCharacteristic', title: '耗材特性', width: 160, slots: { default: 'assetCharacteristic' } },
        { field: 'assetPhysicalState', title: '耗材物态', width: 160, slots: { default: 'assetPhysicalState' } },
        ...
        { field: 'ynInviteTender', title: '招标耗材编码', width: 160, slots: { default: 'ynInviteTender', edit: 'ynInviteTender' }, editRender: {} },
  ...
  watch: {
    'formData.accepterMethod': {
      handler(newVal, oldVal) {
        if (newVal != oldVal && newVal == 1 && this.formData.consumableReceiptDetailId) {
          this.init()
        } else {
          this.tableData = []
        }
    ...
    'formData.consumableReceiptDetailId': {
      handler(newVal, oldVal) {
  ...
  methods: {
    init() {
      ...
      this.loading = true
      queryUnShippedByPurchaseApplicationId({ consumableReceiptDetailId: this.formData.consumableReceiptDetailId })
        .then((res) => {
