        ...
        <SearchArea :show-all-search.sync="showAllSearch" class="p-16">
          <div class="flex">
            <div class="searchLeft">
              <el-row :gutter="14">
                <el-col :span="4">
                  <el-input v-model="queryParams.giftDistributeCode" size="mini" placeholder="派发单号" clearable />
                </el-col>
                <el-col :span="4">
        ...
        <el-container>
          ...
          <el-main>
            ...
            >
              ...
              <template #giftDistributeCode="{row}">
                <el-link :underline="false" type="primary" @click="detailHandle(row.giftDistributeId)">{{ row.giftDistributeCode }}</el-link>
              </template>
              ...
              <template v-slot:todo="{ row }">
                <div class="todo">
                  <el-button size="mini" type="text" @click="detailHandle(row.giftDistributeId)">查看</el-button>
                  <el-button v-hasPermi="['asset:receipt:edit']" size="mini" type="text" @click="addOrUpdateHandle(row.giftDistributeId)">修改</el-button>
  ...
  data() {
    return {
      ...
      tableColumn: [
        ...
        { visible: true, field: 'giftDistributeCode', title: '派发单号', fixed: 'left', width: 160, visibleDisabled: true, slots: { default: 'giftDistributeCode' } },
        ...
        { visible: true, field: 'distributeOrgName', title: '派发部门', width: 130 },
  ...
  methods: {
    ...
    detailHandle(id) {
      window.$wujie.props.route({
        path: '/gift/distribute',
        module: 'Gift',
        fullPath: '/gift/distribute/detail',
        title: '礼品派发详情',
        condition: { id }
      })
    },
    handleDelete(row) {
      this.$confirm('是否确认删除资产派发单信息编号为"' + row.giftDistributeCode + '"的数据项？', '', { type: 'warning' }).then(() => {
        delDistribute(row.giftDistributeId).then(() => {
          this.reload()
