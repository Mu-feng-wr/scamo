<template>
  <div>
    <div class="pt-10 mb-10">
      <selectMaterial
        v-if="formData.accepterMethod != 1 && showBtn"
        :query="[{label: 'centralizedBusinessId',value:formData.centralizedBusinessId,text:'业务类别'}]"
        class="pr-10"
        size="mini"
        @confirm="selectMaterialHandler"
      />
      <el-button size="mini">下载资产标准</el-button>
      <el-button size="mini">下载申购资产明细</el-button>
      <el-button v-if="showBtn" size="mini">批量导入</el-button>
      <uploadImg v-if="showBtn" size="mini" :disabled="btnDisabled" class="pl-10 pr-10" @success="uploadImgSuccess" />
      <!-- <setUser size="mini" v-if="showBtn" :disabled="btnDisabled" class="pr-10" @confirm="setUserHandler" /> -->
  ...
  computed: {
    tableColumn() {
      return [
        ...
        { field: 'alterUnitNum', title: '购买数量', width: 120, visible: this.formData.accepterMethod == 1 },
        { field: 'buyQuantity', title: '发货数量', width: 140, headerAlign: 'center', align: 'right', slots: { default: 'buyQuantity', edit: 'buyQuantity' }, editRender: {} },
