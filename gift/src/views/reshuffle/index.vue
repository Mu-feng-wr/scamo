        ...
        <SearchArea :show-all-search.sync="showAllSearch" class="p-16">
          <div class="flex">
            <div class="searchLeft">
              <el-row :gutter="14">
                <el-col :span="4">
                  <el-input v-model="queryParams.giftReshuffleCode" size="mini" placeholder="异动单号" clearable />
                </el-col>
                <el-col :span="4">
                  <el-input v-model="queryParams.user_name" size="mini" placeholder="异动人" clearable />
                </el-col>
                <el-col :span="4">
                  <el-input v-model="queryParams.user_org_name" size="mini" placeholder="异动部门" clearable />
                </el-col>
                <el-col :span="4">
              ...
              <el-row v-show="showAllSearch" :gutter="14" class="mt-10">
                <el-col :span="8">
                  <input-range
                    type="daterange"
                    size="mini"
                    :start-value.sync="queryParams.reshuffleDateStart"
                    :end-value.sync="queryParams.reshuffleDateEnd"
          ...
          <el-main>
            ...
            >
              ...
              <template v-slot:todo="{ row }">
                <div class="todo">
                  <el-button size="mini" type="text" @click="detailHandle(row.giftReshuffleId)">查看</el-button>
                  <el-button v-hasPermi="['asset:receipt:edit']" size="mini" type="text" @click="addOrUpdateHandle(row.giftReshuffleId)">修改</el-button>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      this.$modal.confirm('是否确认删除资产异动单信息编号为"' + row.assetReturnCode + '"的数据项？', '', { type: 'warning' }).then(() => {
        delReshuffle(row.giftReshuffleId).then(() => {
          this.reload()
