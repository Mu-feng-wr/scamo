        ...
        <SearchArea :show-all-search.sync="showAllSearch" class="p-16">
          <div class="flex">
            <div class="searchLeft">
              <el-row :gutter="14">
                <el-col :span="4">
                  <el-input v-model="queryParams.giftScrapCode" size="mini" placeholder="终结单号" clearable />
                </el-col>
                <el-col :span="4">
                ...
                <el-col :span="4">
                  <el-input v-model="queryParams.scrapReason" size="mini" placeholder="终结原因" clearable />
                </el-col>
              </el-row>
      ...
      <el-main style="padding:0;">
        <el-container>
          <el-header>
            <el-row class="mb-15">
              <el-col :span="12">
                <el-button v-hasPermi="['srm:scrap:add']" type="primary" plain icon="el-icon-plus" size="mini" @click="addOrUpdateHandle()">新增终结</el-button>
                <el-button v-hasPermi="['srm:scrap:export']" plain icon="el-icon-upload2" size="mini" @click="handleExport">导出</el-button>
              </el-col>
          ...
          <el-main>
            ...
            >
              ...
              <template #giftScrapCode="{row}">
                <el-link :underline="false" type="primary" @click="detailHandle(row.giftScrapId)">{{ row.giftScrapCode }}</el-link>
              </template>
              ...
              <template v-slot:todo="{ row }">
                <div class="todo">
                  <el-button size="mini" type="text" @click="detailHandle(row.giftScrapId)">查看</el-button>
                  <el-button v-hasPermi="['asset:scrap:edit']" size="mini" type="text" @click="addOrUpdateHandle(row.giftScrapId)">修改</el-button>
  ...
  data() {
    return {
      ...
      tableColumn: [
        ...
        { visible: true, field: 'giftScrapCode', title: '终结单号', fixed: 'left', width: 160, visibleDisabled: true, slots: { default: 'giftScrapCode' } },
        { visible: true, field: 'scrapDate', title: '终结日期', fixed: 'left', width: 130, visibleDisabled: true },
        ...
        { visible: true, field: 'scrapDate', title: '终结日期', width: 130 },
        { visible: true, field: 'scrapReason', title: '终结原因', width: 130 },
        { visible: true, field: 'totalRecovereResidualValue', title: '累计回收残值金额（元）', width: 130 },
        { visible: true, field: 'totalRecovereResidualValue', title: '累计总终结礼品金额（元）', width: 130 },
      ...
      filterOptions: [
        { label: '终结人：', type: 'baseInput', baseCode: 'companyList', resultLabel: 'data', labelName: 'deptName', valueName: 'deptId', placeholder: '终结人', value: 'assetCompanyId' },
        { label: '终结部门：', type: 'baseInput', baseCode: 'companyList', resultLabel: 'data', labelName: 'deptName', valueName: 'deptId', placeholder: '终结部门', value: 'companyId' },
        ...
        { label: '商名称：', type: 'baseInput', baseCode: 'companyList', resultLabel: 'data', labelName: 'deptName', valueName: 'deptId', placeholder: '商名称', value: 'companyId' },
        ...
        { label: '终结日期期间：', type: 'daterange', placeholder: '请选择终结日期期间', valueStart: 'scrapDateStart', valueEnd: 'scrapDateEnd' },
        { label: '终结状态：', type: 'baseInput', baseCode: 'AlmAssetReceipt-status', options: this.dictDataList, value: 'status' }
      ]
    }
  },
  created() {
  ...
  methods: {
    ...
    detailHandle(id) {
      window.$wujie.props.route({
        path: '/gift/scrap',
        module: 'Gift',
        fullPath: '/gift/scrap/detail',
        title: '礼品处置详情',
        condition: { id }
      })
    },
    handleDelete(row) {
      this.$confirm('是否确认删除资产归还单信息编号为"' + row.giftScrapCode + '"的数据项？', '', { type: 'warning' }).then(() => {
        delScrap(row.giftScrapId).then(() => {
          this.reload()
