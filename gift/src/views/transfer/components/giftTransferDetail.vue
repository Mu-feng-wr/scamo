<template>
  <div>
    <div class="pt-10 mb-10">
      ...
      <!-- <uploadImg size="mini" v-if="showBtn" :disabled="btnDisabled" class="pl-10 pr-10" @success="uploadImgSuccess" /> -->
      ...
      <!-- <selectShelf size="mini" v-if="showBtn" :disabled="btnDisabled" :query="{parentId:formData.warehouseId}" class="pr-10" @confirm="selectShelfHandler" /> -->
...
export default {
  ...
  watch: {
    'formData.accepterMethod': {
      handler(newVal, oldVal) {
        if (newVal != oldVal && newVal == 1 && this.formData.giftTransferDetailId) {
          this.init()
        } else {
          this.tableData = []
        }
  ...
  methods: {
    init() {
      ...
      this.loading = true
      queryUnShippedByPurchaseApplicationId({ giftTransferDetailId: this.formData.giftTransferDetailId })
        .then((res) => {
