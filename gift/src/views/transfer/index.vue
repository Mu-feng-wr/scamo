        ...
        <SearchArea :show-all-search.sync="showAllSearch" class="p-16">
          <div class="flex">
            <div class="searchLeft">
              <el-row :gutter="14">
                <el-col :span="4">
                  <el-input v-model="queryParams.giftTransferCode" size="mini" placeholder="调拨单号" clearable />
                </el-col>
                <el-col :span="4">
                ...
                <el-col :span="4">
                  <el-input v-model="queryParams.transferReason" size="mini" placeholder="调拨原因" clearable />
                </el-col>
              </el-row>
              ...
              <el-row v-show="showAllSearch" :gutter="14" class="mt-10">
                <el-col :span="8">
                  <input-range
                    type="daterange"
                    size="mini"
                    :start-value.sync="queryParams.transferDateStart"
                    :end-value.sync="queryParams.transferDateEnd"
                    start-placeholder="调拨日期开始"
                    end-placeholder="调拨日期结束"
        ...
        <el-container>
          ...
          <el-main>
            ...
            >
              ...
              <template #giftTransferCode="{row}">
                <el-link :underline="false" type="primary" @click="detailHandle(row.giftTransferId)">{{ row.giftTransferCode }}</el-link>
              </template>
              ...
              <template v-slot:todo="{ row }">
                <div class="todo">
                  <el-button size="mini" type="text" @click="detailHandle(row.giftTransferId)">查看</el-button>
                  <el-button v-hasPermi="['asset:receipt:edit']" size="mini" type="text" @click="addOrUpdateHandle(row.giftTransferId)">修改</el-button>
...
<script>
import { listTransfer, delTransfer } from '@/api/transfer.js'
  ...
  data() {
    return {
      ...
      tableColumn: [
        ...
        { visible: true, field: 'giftTransferCode', title: '调拨单号', fixed: 'left', width: 160, visibleDisabled: true, slots: { default: 'giftTransferCode' } },
        { visible: true, field: 'transferDate', title: '调拨日期', fixed: 'left', width: 130, visibleDisabled: true },
        ...
        { visible: true, field: 'outCompanyName', title: '调出公司', width: 130 },
        { visible: true, field: 'inCompanyName', title: '调入公司', width: 130 },
        { visible: true, field: 'transferReason', title: '调拨原因', width: 130 },
        { visible: true, field: 'outProjectCode', title: '调出项目编号', width: 130 },
        { visible: true, field: 'outProjectName', title: '调出项目名称', width: 130 },
        { visible: true, field: 'inProjectCode', title: '调入项目编号', width: 130 },
        { visible: true, field: 'inProjectName', title: '调入项目名称', width: 130 },
      ...
      filterOptions: [
        ...
        { label: '调出公司：', type: 'baseInput', baseCode: 'companyList', resultLabel: 'data', labelName: 'deptName', valueName: 'deptId', placeholder: '调出公司', value: 'companyId' },
        { label: '调入公司：', type: 'baseInput', baseCode: 'companyList', resultLabel: 'data', labelName: 'deptName', valueName: 'deptId', placeholder: '调入公司', value: 'companyId' },
        { label: '调出项目：', type: 'baseInput', baseCode: 'listContract', labelName: 'contractName', valueName: 'contractId', value: 'contractId' },
        { label: '调入项目：', type: 'baseInput', baseCode: 'listContract', labelName: 'contractName', valueName: 'contractId', value: 'contractId' },
        { label: '使用终端：', type: 'baseInput', baseCode: 'System-sourceTerminal', value: 'sourceTerminal' },
        { label: '调拨状态：', type: 'baseInput', baseCode: 'AlmAssetReceipt-status', options: this.dictDataList, value: 'status' },
  ...
  methods: {
    ...
    detailHandle(id) {
      window.$wujie.props.route({
        path: '/gift/transfer',
        module: 'Gift',
        fullPath: '/gift/transfer/detail',
        title: '礼品调拨详情',
        condition: { id }
      })
    },
    handleDelete(row) {
      this.$confirm('是否确认删除资产调拨单信息编号为"' + row.giftTransferCode + '"的数据项？', '', { type: 'warning' }).then(() => {
        delTransfer(row.giftTransferId).then(() => {
          this.reload()
    ...
    getFooterData({ columns, data }) {
      return this.footerMethod(columns, data, '', ['assetQuantity', 'giftAmount'])
    },
    // 获取字典数据
    getDictData() {
