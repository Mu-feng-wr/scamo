<template>
  <SectionCard title="安全IP设置" :isHeaderSplit="true">
    <vxe-grid
      ref="xTable"
      v-loading="loading"
      header-align="center"
    ...
    >
      ...
      <template #ip="{row}">
        <el-input v-if="$refs.xTable.isActiveByRow(row)" v-model="row.ip" size="small" placeholder="ip"></el-input>
      </template>
    ...
    <el-button icon="el-icon-circle-plus-outline" class="btn-block" style="margin-top: 15px" @click="addData()">添加安全IP</el-button>
  </SectionCard>
</template>

<script>
import { listIp, delIp, updateIp } from '@/api/ip.js'
export default {
  data() {
    return {
      loading: false,
      ...
      tableColumn: [
        ...
        { field: 'ip', title: 'ip', minWidth: 100, slots: { edit: 'ip' }, editRender: {} },
  ...
  methods: {
    load() {
      this.loading = true
      listIp({ customerId: this.$store.getters.userInfo.customerId })
        .then((res) => {
          this.cacheIpList = JSON.parse(JSON.stringify(res.rows))
    ...
    dataFormClose() {
      this.$refs.xTable.clearActived()
      this.ipList = JSON.parse(JSON.stringify(this.cacheIpList))
    },
    handleDelete(row) {
      this.$confirm('是否确认删除安全IP设置信息编号为"' + securityIpIds + '"的数据项？', '', { type: 'warning' }).then((res) => {
        delIp(row.securityIpId).then((res) => {
          this.load()
